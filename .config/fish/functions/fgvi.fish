function fgvi
  ...
  if test $compare_remote -eq 1
    set target_file (git diff --name-only --diff-filter=ACMR remotes/origin/(git_branch_name) | fzf -e)
    ...
    set target_file (git diff --name-only --diff-filter=ACMR $target_branch..$current_branch | fzf -e)
    set target_path (git rev-parse --show-toplevel)"/"$target_file
  end
