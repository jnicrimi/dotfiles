...
1. 取得したローカルブランチ一覧からカレントブランチを除外してください
1. 未コミットの変更がある場合はリストの最後に「未コミットの変更」オプションを追加してください

#### 3.3 レビュー対象の判定と表示

##### レビュー対象が存在しない場合

- 「レビュー可能な変更がありません」と出力して処理を終了してください
...
選択されたブランチとの差分を取得してください

- `${selected_branch}` に <選択されたブランチ名> を設定します
- `${review_target}` に 「対象のブランチ: <選択されたブランチ名>」を設定します
...
- **MUST**: code-reviewerエージェントを使用してください
- **MUST**: 選択された思考レベルで分析の深さを決定してください
- **MUST**: レビューチェックリストの全カテゴリを分析してください
- **CRITICAL**: エージェントへの指示に以下を必ず含めてください：
...
{{! 1. SQL注入の脆弱性 }}
  ...
  ◆ 影響範囲
    • [この問題による影響]
    {{! • データベースの不正操作、情報漏洩、データの改ざんリスク }}
  ◆ 修正方法
    • [推奨される修正方法]
    {{! • プリペアドステートメントを使用するか、入力値のサニタイズを実装する }}
...
{{! 3. テストカバレッジ不足 }}
  ...
  ◆ 問題内容
    • [詳細な問題の説明]
    {{! • 決済処理の重要な機能にテストが存在しない }}
  ◆ 修正方法
    • [推奨される修正方法]
    {{! • ユニットテストとエッジケースのテストを追加する }}
...
{{! 6. 非推奨APIの使用 }}
  ◆ ファイル
    • [ファイル名:行番号]
    {{! • legacy/client.js:156 }}
  ◆ 問題内容
    • [詳細な問題の説明]
    {{! • 廃止予定の外部APIを使用しており、将来的に動作しなくなる可能性 }}
  ◆ 修正方法
    • [推奨される修正方法]
    {{! • 新しいAPIバージョンへの移行計画を立てる }}
...
{{! 7. 適切なエラーハンドリング }}
  ...
  ◆ 評価理由
    • [なぜこれが良い実装なのか]
    {{! • エラー時でもアプリケーションが安定して動作し、ユーザーが問題を理解しやすい }}
